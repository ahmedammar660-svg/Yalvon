...
def refresh_technicals_cache(symbols):
    """Recompute technicals for every symbol once per bulk-refresh
    cycle (every BULK_REFRESH_MINUTES) instead of on every request —
    computing this for ~500 symbols on each page load would be slow."""
    fresh = {}
    for symbol in symbols:
        try:
            fresh[symbol] = compute_technicals(symbol)
        except Exception:
            pass
...
def get_bulk_quotes(force=False):
    ...
    if force or (stale and not has_items):
        # First call (or a forced refresh): fetch synchronously so the
        # caller gets real data immediately rather than an empty list.
        refresh_bulk_quotes()
    elif stale:
...
def screener_run():
    ...
    # Only pay the per-symbol technicals-computation cost when the
    # person actually asked for a technical filter.
    if TECHNICAL_CRITERIA_KEYS & set(criteria.keys()):
        merged = []
        for q in items:
            symbol = q.get("symbol")
            technicals = get_cached_technicals(symbol) if symbol else {}
            merged.append({**q, **{k: v for k, v in technicals.items() if k != "symbol"}})
        items = merged
...
# Run once on import, not just under `python app.py` — this matters
# because a production WSGI server (gunicorn, etc.) imports this file
# as a module and calls the `app` object directly; it never executes
# the `if __name__ == "__main__":` block below. Without this, deploying
# behind gunicorn would silently skip table creation and the background
# price-refresh thread entirely — the app would "run" but every request
# touching the database would fail, and prices would never update.
init_db()
...
if __name__ == "__main__":
    ...
    # In production, a WSGI server like gunicorn imports `app` directly
    # and this block is never reached — see init_db()/start_bulk_refresh_thread()
    # calls above, which run unconditionally on import instead.
